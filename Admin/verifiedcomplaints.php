...
<head>
    ...
    <title>ADMIN VERIFIED COMPLAINTS</title>
...
<div class="event">
            <div>
                <h1 class="event_form_heading">Verified Registered</h1>
            </div>
...
while($row=$result->fetch_assoc()){
                    ...
                    echo "<td><button class='edit'><a href='c-verified.php?id=$id' class='btn btn-primary btn-sm'>View</a></button></td>";
                    echo "<td><button class='delete'><a href='delete.php?id=$id' class='btn btn-success btn-sm'>Delete</a></button></td>";
                    if($row['isVf']=='verified'){
                        $state= "<p style='color: green;'> Un-Verify</p>";
                        $cls="btn-disabled";
                    ...
                    echo "<td><button class='verify' ><a href='unverify.php?id=$id' class='$cls'>$state</button></td>";
                    //echo "<td>".$row['isVf']."</td>";
                  //  echo "<td>".$etime12."</td>";
...
// echo $hall;
$dep = $_POST['dep'];
$sql = "SELECT * FROM complaint WHERE hall= '$shall' AND  sname  LIKE '%$hall%' AND sname LIKE '%$dep%' ";
                    $result = $conn->query($sql);
